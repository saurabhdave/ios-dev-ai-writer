...
### Added
- **Incremental learning: recurring failures are mined from history and fed back into the prompts** (`utils/learnings.py`, `agents/article_agent.py`, `agents/code_agent.py`, `config.py`, `README.md`, `tests/test_learnings.py`): The pipeline already *recorded* every run's review critiques and type-check failures in `quality_history.json` but only used them for dashboards/regression alerts — a human had to notice a recurring ding and hand-edit a prompt. Now `utils/learnings.py` mines the recent window (`LEARNINGS_WINDOW`, default 25), surfaces patterns that recur at least `LEARNINGS_MIN_COUNT` times (default 2; one-offs ignored), and builds two bounded, deterministic "avoid these" digests that are prepended to the prompts: a **code** digest of recurring inline type-check errors (wrong labels, nonexistent members, conformance gaps — tooling noise like timeouts filtered out) injected into the code-generation prompt, and an **editorial** digest of recurring review/layout critiques (order-independent keyword fingerprint clusters the reviewer's consistent phrasing) injected into the article prompt. Both return `""` until something actually recurs, so it is a safe no-op on a fresh history; gated by `LEARNINGS_INJECTION_ENABLED` (default `true`). On the live 63-run history the editorial digest already surfaces three standing critiques (duplicate Validation/Observability sections, missing pull-quote, plain-text closing takeaway); the code digest populates as type-check failures accumulate under the new validation.
- **Stub-tolerant, multi-SDK Swift type-checker** (`agents/swift_validation.py`, `tests/test_swift_validation.py`): A new validator that compiles a snippet with `swiftc -typecheck` against the SDK matching the frameworks it references (iOS / macOS / watchOS), injects an SDK-appropriate import preamble so import-less fragments resolve their framework types (without pulling in modules that make a common symbol ambiguous — e.g. RealityKit would shadow SwiftUI's `Scene`), and ignores "cannot find … in scope" diagnostics (and any cascade naming a stub) so illustrative snippets that reference undefined helpers are not falsely failed. What survives is genuine API misuse — wrong argument labels, nonexistent members, illegal declarations (`@MainActor actor`), invented types (`Task.Handle`). `@main` blocks get `-parse-as-library` and `import XCTest` blocks get the platform framework search path. Actor-isolation / global-mutable-state diagnostics are classified separately (mode-sensitive, non-blocking). Degrades to a no-op when `swiftc`/`xcrun` or the SDK is absent.

### Changed
- **Generated Swift is now type-checked, not just parsed — and validation actually runs in CI** (`agents/code_agent.py`, `workflows/weekly_pipeline.py`, `config.py`, `.github/workflows/weekly.yml`, `README.md`): Two compounding gaps let API hallucinations reach published articles. (1) Inline body snippets were only parsed (`swiftc -frontend -parse`) — syntax-only — so semantically-wrong-but-syntactically-valid misuse (`OSSignposter.endInterval(name, id:)`, RealityKit `Entity.destroy()`, `@MainActor actor`, `@Parameter var x: UUID`) passed validation. (2) The `generate-article` job ran on `ubuntu-latest`, where `swiftc` is absent — so *every* Swift validator hit its `if not swiftc: return True, ""` guard and silently no-oped: **no generated Swift was ever actually compiled in production.** `validate_inline_snippets()` now runs the stub-tolerant type-check after the parse pass and, when `repair=True` (wired into the weekly pipeline), repairs a failing block once and replaces it in place only if the repair type-checks and does not balloon the fragment (≤ max(12, 3×) lines). `_swift_compile_validate()` (the standalone-example path) routes by SDK instead of iOS-only, so AppKit / HealthKit examples are checked against the macOS / watchOS SDK. `CODEGEN_VALIDATION_MODE` now defaults to `compile` (was `snippet`), and `generate-article` moved to `macos-latest` so the toolchain is present. Parse-failure reporting and the existing `@Bindable` / banned-API checks are unchanged, so the existing inline-snippet tests still hold.
- **Unrepairable inline snippets are stripped, never published broken** (`agents/code_agent.py`, `config.py`, `README.md`, `tests/test_swift_validation.py`): LLM repair is best-effort — deep cases (nonexistent methods, full protocol conformance) don't reliably fix in a few passes. So when an inline block still fails to type-check after the repair loop, the fenced block is now removed (the surrounding prose is kept and blank-line runs collapsed), guaranteeing no broken inline Swift reaches the content repo. The issue is still recorded for observability and an `inline_snippet_stripped` event is logged. Gated by `CODEGEN_STRIP_UNREPAIRABLE_INLINE` (default `true`); detection-only callers (`repair=False`, e.g. tests) never strip. End-to-end verified: the test article's 6 blocks → repaired 1, stripped 2, and the published result type-checks 100% clean.
- **Inline repair is iterative, and token budgets sized for GPT-5 reasoning** (`agents/code_agent.py`): A failing inline block usually has multiple errors where swiftc only reports the first (later ones are suppressed cascades), so a single repair pass fixed one and the re-check surfaced the next, leaving the block broken. Inline repair now loops up to `MAX_INLINE_REPAIR_ATTEMPTS` (3), feeding each new diagnostic back, with the candidate size still bounded against the original. Separately, `REPAIR_MAX_TOKENS` (1_400→4_000) and `CODEGEN_MAX_TOKENS` (1_200→3_000) were raised because GPT-5 reasoning tokens count against `max_output_tokens` — the old budgets truncated full rewrites into unparsable Swift. End-to-end verification on a real run showed the loop now detects every hallucination (3 in the test article that would previously have shipped silently) and repairs cascading-error blocks; deep cases (nonexistent methods, protocol conformance) remain best-effort.
- **Generation model raised to `gpt-5`** (`config.py`, `.github/workflows/weekly.yml`, `README.md`): `OPENAI_MODEL` default and the weekly workflow env bumped from `gpt-5-mini` to `gpt-5` for stronger first-pass code and prose and fewer repair rounds; still overridable via `OPENAI_MODEL`. GPT-5 reasoning-effort / temperature handling was already in place.

### Fixed
- **Writer/reviewer version-guidance contradiction resolved with a deterministic baseline note** (`utils/article_repair.py`, `workflows/weekly_pipeline.py`, `prompts/article_prompt.txt`, `prompts/review_prompt.txt`, `prompts/review_repair_prompt.txt`, `tests/test_article_repair.py`): The article/editor prompts forbid version callouts for baseline APIs ("assume iOS 18 / Swift 6") while the review prompt asked "Are Swift version constraints stated?" and listed "version claims without stated deployment target" as an issue — so 7 of the last 8 reviews docked the same point and articles plateaued at 8/10. `ensure_version_baseline_note()` now deterministically inserts "*All code in this article targets iOS 18+ and Swift 6.2 unless noted otherwise.*" after the hook intro during the deterministic-repair step (idempotent; skipped when the article already states a baseline; Swift version from `SWIFT_LANGUAGE_VERSION` trimmed to major.minor), and re-inserts it after a review-triggered repair in case the repair LLM drops it. The review prompt now treats the note as satisfying deployment-target guidance and only flags APIs newer than the stated baseline; the review-repair prompt is instructed to preserve the note verbatim. The note is platform-aware: `_dominant_platform_baseline()` detects the article's platform from body mentions (iOS default and tie-winner) — the first WWDC-mode run's macOS article was flagged by self-review for carrying an "iOS"-labelled note. The baseline generation is config-driven via `APPLE_OS_BASELINE` (default `26` — the latest *shipped* OS generation, never the current beta; Apple's unified numbering means one value covers every platform). The article/editor/code prompts were bumped from the stale iOS 18 baseline to iOS 26 in the same change, with an explicit rule never to present beta-only APIs as shippable.
- **Article, LinkedIn post, codegen, and newsletter now stay topic-synced in the content repo** (`.github/workflows/weekly.yml`): The publish step's same-date replacement logic only covered `articles/` — when a same-date re-run replaced an article (e.g. a `workflow_dispatch` with a new forced topic), the LinkedIn post, codegen metadata, and newsletter issue from the replaced topic survived in `ios-ai-articles`, leaving artifacts for different topics on the same date (observed: orphaned `2026-03-31-speed-up-xcode-builds-with-explicit-modules-linkedin.md`). A shared `sync_artifacts` function now applies the same-date cleanup to all four artifact directories, comparing file stems so the newsletter's paired `.md`/`.html` files don't evict each other.
- **RSS feeds are now fetched through the retrying session with a timeout** (`scanners/trend_scanner.py`, `tests/test_trend_scanner_feeds.py`): `_parse_rss_feed()` previously handed the feed URL straight to `feedparser.parse()`, which fetches via urllib with **no timeout and no retries** — bypassing the retry-configured `requests.Session` entirely and allowing a single hung feed to stall the whole pipeline run. Feeds are now fetched with `_SESSION.get(url, timeout=TREND_HTTP_TIMEOUT_SECONDS)` (3 retries, exponential backoff) and the response bytes are passed to feedparser; network failures log `feed_fetch_error` and return an empty list as before.
- **Cross-repo dedup fetch is now authenticated in CI** (`workflows/weekly_pipeline.py`, `.github/workflows/weekly.yml`, `tests/test_published_titles_auth.py`): `_load_published_titles()` hit the GitHub Contents API unauthenticated, sharing the 60/hour per-IP limit that GitHub Actions runner pools routinely exhaust — so the defense-in-depth dedup check could silently 403 in CI. A new `_github_api_request()` helper attaches `Authorization: Bearer $GITHUB_TOKEN` when the env var is set, and the workflow now passes `secrets.GITHUB_TOKEN` to the pipeline step.
- **Health check no longer misreports lifetime totals or false-trips on unscored runs** (`scripts/health_check.py`, `tests/test_health_check.py`): The regression issue body printed `len(quality_history.json)` as "Total runs", which freezes at the 200-entry cap once trimming starts — it now reads `quality_history_meta.json` via `_lifetime_total_runs()` with a fallback to the window length. Average review scoring (`_avg_review_score()`) now ignores entries without a numeric `review_overall` instead of counting them as 0, so a stretch of `SELF_REVIEW_ENABLED=false` runs can no longer drag the average below `HEALTH_MIN_AVG_REVIEW` and file a phantom regression; when no entry in the window is scored, the review threshold is skipped entirely.

- **Family-classifier regex stems never matched inflected forms** (`scripts/update_readme.py`): The `_FAMILY_RULES` stems carried a trailing word boundary, so `concurrenc\b` could not match `concurrency` (the next character is a word char) — sending ~27% of titles to the `other` bucket and distorting the README family-coverage metrics. Switched all stems to `\w*` suffixes (`concurren\w*`, `migrat\w*`, `architectur\w*`, …) and added missing signals (`realitykit`/`arkit`, `on-device`, `hang detection`, `view builder`).
...
- **Deterministic `@Bindable` stripping from `@Observable` bodies** (`utils/article_repair.py`, `agents/code_agent.py`): Added `strip_bindable_from_observable()` to `article_repair.py` — a pure regex pass that removes `@Bindable` annotations from stored properties inside `@Observable` classes before snippet validation. `code_agent.py` now calls this helper after initial generation and after each LLM repair pass (the model frequently re-introduces the same mistake). This directly addresses the ~30% codegen omit rate caused by the LLM repair loop failing on this exact diagnostic. Auto-repair count is recorded in `codegen.json` as `auto_repaired_bindable`.

### Removed
- **Stray content-repo Jekyll config** (`ios-ai-articles/_config.yml`): Leftover from the output-repo migration — the file's own header noted it belongs in `saurabhdave/ios-ai-articles`, which has carried its own (richer) `_config.yml` since. Removed from this repo.

### Added
- **`multi_per_day` workflow input — publish several articles on one day** (`.github/workflows/weekly.yml`, `README.md`): The publish step's same-date replacement (which keeps scheduled re-runs from leaving orphaned artifacts) made it impossible to publish more than one article per day — each manual run evicted the previous run's article, LinkedIn post, codegen metadata, and newsletter issue from the content repo. A new `workflow_dispatch` boolean input `multi_per_day` (default `false`) skips the same-date cleanup in `sync_artifacts` so artifacts accumulate instead — built for WWDC coverage bursts. Scheduled runs are unaffected and keep the replacement protection. `scripts/publish_burst.py` streamlines bursts: it triggers sequential `workflow_dispatch` runs (each with `multi_per_day=true`, sequential to avoid state races) from a topic list or file, validates the 60-char/10-word title limits up front, and waits for each run before starting the next.
- **Reference-content injection — agents now see real page content, not just titles** (`utils/reference_content.py`, `config.py`, `agents/article_agent.py`, `agents/editor_agent.py`, `prompts/article_prompt.txt`, `prompts/article_factuality_prompt.txt`, `workflows/weekly_pipeline.py`, `tests/test_reference_content.py`): The pipeline passed only reference titles+URLs to the agents, so timely topics (WWDC announcements, new OS releases) produced evergreen-generic articles — the model's training data predates the news. A new `fetch_reference_excerpts()` fetches the top trusted reference pages (Apple/Swift.org preferred), extracts visible text with a stdlib-only HTML parser (JS-rendered SPA shells are detected by thin output and skipped), and injects titled, URL-free excerpts into the article and factual-grounding prompts; the factuality pass treats claims consistent with the excerpts as supported evidence. Controlled by `REFERENCE_CONTENT_ENABLED` (default `true`), `REFERENCE_CONTENT_MAX_PAGES` (3), and `REFERENCE_CONTENT_MAX_CHARS` (1500/page).
- **Annual WWDC mode — daily announcement coverage during the conference window** (`config.py`, `agents/topic_agent.py`, `.github/workflows/weekly.yml`, `tests/test_wwdc_mode.py`): `WWDC_START_DATE`/`WWDC_END_DATE` (set yearly in `weekly.yml` env — never auto-guessed) define the window; `wwdc_window_active()` gates all behavior. In-window: a new daily cron (`0 10 * * 0,2,3,5,6`) is admitted by the `wwdc-gate` job (outside the window those runs exit in seconds; Mon/Thu and manual runs are never gated), topic generation suspends the family rotation and platform-diversity/novelty gates (only exact-duplicate titles are rejected, since daily WWDC titles necessarily share the "WWDC <year>" token), and each day covers a different angle (keynote → SwiftUI → Swift/concurrency → tooling → platforms → Apple Intelligence) with an announcement-summary editorial guardrail — never deep tutorials on brand-new APIs, the year's highest hallucination risk. `wwdc` was also added to the topic agent's Apple-word patterns so WWDC titles pass the platform gate.
- **Inline body snippets are now validated** (`agents/code_agent.py`, `workflows/weekly_pipeline.py`, `tests/test_wwdc_mode.py`): Inline ```swift blocks written by the article/editor LLMs were never validated (only the standalone appended example was) — review history shows won't-compile bugs slipping through. New `validate_inline_snippets()` applies the deterministic `@Bindable`-in-`@Observable` fix inside body fences and runs the syntax-only swiftc parse check per block (no-op when swiftc is unavailable). Diagnostic, never blocking: issues are logged as `inline_snippet_issues_detected` and recorded per-run in `quality_history.json` (`inline_snippet_issues`).
- **Inline code snippets are now enforced by the layout rubric** (`agents/editor_agent.py`, `tests/test_layout_rubric.py`): The article prompt has long required inline ```swift snippets in the body, but nothing enforced it and LLM compliance was imperfect — while quality-history analysis shows missing inline code is the single biggest editorial-quality lever. `assess_medium_layout()` gains a 15th check: at least `INLINE_CODE_BLOCK_MIN` (1) fenced code block in the article body, scored as 1 point AND flagged as a *required* issue, so the existing layout repair loop now deterministically re-prompts until an inline snippet is present (or `MEDIUM_LAYOUT_MAX_REPAIR_PASSES` is exhausted). The check runs before `code_agent` appends the standalone example, so every fence it counts is genuinely inline. `LAYOUT_MAX_SCORE` is now 15; `layout_max_score` in `quality_history.json` records the new maximum automatically.
- **`TREND_FETCH_MAX_WORKERS` env var** (`config.py`): Caps the trend-discovery thread pool (default 8); set to `1` to restore fully sequential fetching for debugging. (`scripts/health_check.py`, `.github/workflows/weekly.yml`): New `scripts/health_check.py` runs after the pipeline (and after `scripts/update_readme.py`) and trips when any of three thresholds regress over their rolling window — codegen success rate (`HEALTH_MIN_CODEGEN_SUCCESS_PCT`, default 70, last 10 runs), zero-coverage families (`HEALTH_MAX_ZERO_COVERAGE_FAMILIES`, default 4, last 8 picks), and average review score (`HEALTH_MIN_AVG_REVIEW`, default 7.5, last 10 runs). On a trip it writes a Markdown body to `outputs/health_regression.md` and exits non-zero; the `File health-regression issue` CI step then files a `pipeline-health-regression` issue, or comments on the existing open one to avoid duplicate-issue spam. Exits 0 (CI no-op) when healthy or when there is not enough history yet. Skipped on `dry_run`.
- **Manual-run overrides via `workflow_dispatch`** (`.github/workflows/weekly.yml`, `agents/topic_agent.py`): The workflow now accepts `forced_topic` (override the generated title, length-constrained, skips LLM topic generation), `forced_family` (`auto` or a known family name to bypass the weighted sampler; unknown names are ignored with a warning), and `dry_run` (run the pipeline but skip README regen, state persistence, and publishing). `topic_agent.py` honors the `FORCED_TOPIC` and `FORCED_FAMILY` env vars these inputs map to. Scheduled runs are unaffected.
- **Failure auto-issue** (`.github/workflows/weekly.yml`): A new `Open issue on failure` step uses the `gh` CLI and the built-in `GITHUB_TOKEN` to file a labeled issue with the run URL, trigger, and `outputs/run_summary.json` when the pipeline fails.
- **Dependabot** (`.github/dependabot.yml`): Weekly `pip` and `github-actions` update checks on Monday 06:00 UTC — four hours before the pipeline so bumped dependencies are exercised the same day.
- **Zero-coverage boost for topic family rotation** (`agents/topic_agent.py`): Families with zero picks in the last 8 runs receive a `ZERO_COVERAGE_BOOST` weight (~3× `BASE_WEIGHT`) so under-covered families dominate random sampling when several are simultaneously unseen. Previously the rotation could cycle through the same 2–3 families for many consecutive runs when their weights only partially decayed.
- **Two-sided platform-diversity gate** (`agents/topic_agent.py`): The platform mix is bounded on both sides via `_platform_diversity_mode()` (the single source of truth for both the prompt warning and `_violates_platform_diversity()`). When fewer than 2 of the last 8 articles mention a non-iOS Apple platform (macOS, watchOS, visionOS, iPadOS, tvOS, RealityKit, ARKit, WidgetKit, AppKit, CarPlay, Mac Catalyst), the next title is forced non-iOS; when MORE than 5 of the last 8 are non-iOS, the next title is forced back toward iOS (otherwise the gate over-corrected — observed 7/8 non-iOS after 2026-05-07, steering codegen into weak AppKit territory). Cross-platform titles (mentioning both iOS and a non-iOS platform) pass either gate; only pure non-iOS is rejected in iOS-focus mode (`_mentions_ios_platform()`).
- **Persistent topic family pick history** (`agents/topic_agent.py`, `memory/family_picks.json`, `.github/workflows/weekly.yml`, `.gitignore`): `_record_family_pick()` appends the chosen family name to `memory/family_picks.json` after each run (max 256 entries, newest last). CI auto-commits this file alongside `quality_history.json` so the rotation state survives across GitHub Actions runs. Seeded with the chronological family classification of 22 articles published 2026-03-14 through 2026-05-07.

### Changed
- **Trend discovery now runs sources in parallel and HackerNews in a single request** (`scanners/trend_scanner.py`, `config.py`, `tests/test_trend_discovery.py`): `discover_ios_trends()` previously ran every source fetcher sequentially — and HackerNews alone issued up to 1+80 serial Firebase requests, with the viral source adding 16 serial Google News queries. Sources now fan out across a `ThreadPoolExecutor` (capped by `TREND_FETCH_MAX_WORKERS`, default 8), and the HackerNews fetcher uses one Algolia search request (`tags=story`, last 48 h, 80 candidates) that returns titles, points, and comment counts in a single payload — same filtering and scoring formula as before. Results are collected in source listing order so dedup precedence is identical to the sequential implementation, and per-source exception isolation is preserved (`_fetch_source()`). Full discovery measured locally at ~9 s versus the previous minutes-long sequential sweep.
- **AI-exclusion filters extracted to one shared module** (`utils/content_filters.py`, `scanners/trend_scanner.py`, `workflows/weekly_pipeline.py`, `agents/topic_agent.py`, `tests/test_content_filters.py`): Three near-identical copies of the generic-AI exclusion patterns and the Apple Intelligence allowlist lived in the trend scanner (`_EXCLUSION_PATTERNS`/`_INTELLIGENCE_ALLOWLIST`), the pipeline (`REFERENCE_EXCLUSION_PATTERNS`/`REFERENCE_ALLOWED_INTELLIGENCE_PATTERNS`), and the topic agent (`AI_WORD_PATTERNS`/`APPLE_INTELLIGENCE_ALLOWLIST`) — and had already drifted once when a fix landed in only one copy. New `utils/content_filters.py` owns the patterns plus three helpers (`matches_ai_exclusion`, `has_allowed_intelligence_context`, `is_excluded_ai_topic`); all three modules now import from it. Behaviour is unchanged, including the topic agent's supplemental-interest filter, which intentionally applies the raw exclusion without the allowlist.
- **Claude Code guidance files are now local-only, not tracked** (`.gitignore`): Removed `CLAUDE.md` from version control and gitignored all Claude context files — `CLAUDE.md`, `CLAUDE.local.md`, and the `docs/` companion directory imported by `CLAUDE.md` (`architecture.md`, `ci.md`, `configuration.md`) — so Claude Code guidance is kept per-machine rather than committed. The files still live in the working tree locally; they are simply no longer pushed.
- **Recent-title embeddings cached across the generation retry loop** (`agents/topic_agent.py`): Hoisted recent-title embedding out of `_is_semantically_repetitive()` into `_embed_recent_titles()`, called once in `generate_topic()` and reused for every candidate attempt. Previously each retry re-embedded the full recent-title set; now only the candidate is embedded per attempt, cutting embedding API calls by up to `MAX_GENERATION_ATTEMPTS - 1`. Falls back to the original batched-embed path when the one-time embed fails, so `_is_repetitive()` remains the safety net.
- **`quality_history.json` bounded to the last 200 runs with a lifetime-totals meta sidecar** (`workflows/weekly_pipeline.py`, `scripts/update_readme.py`, `.github/workflows/weekly.yml`, `.gitignore`, `README.md`): `_append_quality_history()` now caps the file at `QUALITY_HISTORY_MAX_ENTRIES` (200) so it stays bounded — the dashboard, README, and health check only read the trailing window. To keep the README "Total runs" metric and start date honest after trimming, it writes the lifetime `total_runs` and `first_run_date` to `outputs/quality_history_meta.json` before the cap is applied, seeding the count from the still-untrimmed file on first use. `update_readme.py` reads the sidecar with a fallback to `len(history)`/`history[0]` for pre-sidecar runs. CI persists the meta file alongside quality history; `.gitignore` re-includes it.

...
- **Platform-diversity gate** (`agents/topic_agent.py`): When fewer than 2 of the last 8 articles mention a non-iOS Apple platform (macOS, watchOS, visionOS, iPadOS, tvOS, RealityKit, ARKit, WidgetKit, AppKit, CarPlay, Mac Catalyst), a diversity warning is injected into the topic prompt and iOS-only candidates are rejected by `_violates_platform_diversity()`. Prevents long iOS-only streaks in published output.

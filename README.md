...
| `OPENAI_REASONING_EFFORT` | `low` | `minimal`\|`low`\|`medium`\|`high` — for GPT-5 models |
...
Outputs are gitignored locally and auto-published to [saurabhdave/ios-ai-articles](https://github.com/saurabhdave/ios-ai-articles) on every CI run.
...
**Pipeline steps:** checkout → install deps → `python main.py` → publish outputs to content repo → commit trend snapshot and quality history back to this repo.
...
Follows [Semantic Versioning](https://semver.org). Current version: **1.6.0** — see [CHANGELOG.md](CHANGELOG.md).
